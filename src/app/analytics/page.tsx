      ...
      <div className="max-w-7xl mx-auto space-y-10">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
          <div className="p-6 rounded-3xl bg-white border border-slate-200 shadow-sm space-y-2">
            ...
            <div className="text-3xl font-black text-slate-900">{overallMastery}%</div>
            <div className="w-full h-1.5 rounded-full bg-slate-100 overflow-hidden">
              <div className="h-full bg-indigo-600 rounded-full" style={{ width: `${overallMastery}%` }} />
            </div>
          </div>
          ...
          <div className="p-6 rounded-3xl bg-white border border-slate-200 shadow-sm space-y-2">
            ...
            <div className="text-3xl font-black text-slate-900">54s <span className="text-xs text-slate-400">/ Question</span></div>
          ...
          <div className="p-6 rounded-3xl bg-white border border-slate-200 shadow-sm space-y-2">
            ...
            <p className="text-xs text-slate-500 font-medium">Across 5 semester courses</p>
          </div>
          ...
          <div className="p-6 rounded-3xl bg-white border border-slate-200 shadow-sm space-y-2">
            ...
            <div className="text-3xl font-black text-purple-700">96.8%</div>
            <p className="text-xs text-purple-600 font-bold">FSRS v6 Active</p>
          </div>
        </div>
        ...
        <div className="space-y-6">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
              <span className="text-xs font-black uppercase text-indigo-600">Deep Diagnostic</span>
              <h2 className="text-2xl sm:text-3xl font-black text-slate-900">Subject Mastery & Weakness Analysis</h2>
            </div>
          ...
          <div className="space-y-4">
            {filteredData.map((sub, idx) => (
              <motion.div
                ...
                className="p-6 sm:p-7 rounded-3xl border border-slate-200 bg-white space-y-5 shadow-sm hover:border-indigo-300 transition-all"
              >
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                  <div className="space-y-1">
                    <div className="flex items-center gap-3">
                      <h3 className="text-xl font-bold text-slate-900">{sub.subject}</h3>
                      {sub.status === "Weak" ? (
                    ...
                    <p className="text-xs text-slate-500">{sub.totalAttempted} Questions Practiced • {sub.accuracy}% Accuracy</p>
                  </div>

                  <div className="flex items-center gap-6">
                    <div className="text-right">
                      <div className="text-2xl font-black text-slate-900">{sub.masteryPercentage}%</div>
                      <div className="text-[11px] text-slate-400 uppercase font-bold">Mastery Level</div>
                    </div>

                    <Link
                      href="/flashcards"
                      className="px-5 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white font-bold text-xs transition-all shadow-sm flex items-center gap-1.5"
                    >
                ...
                <div className="w-full h-2 rounded-full bg-slate-100 overflow-hidden border border-slate-200">
                  <div 
                    className={`h-full rounded-full ${
                      sub.masteryPercentage >= 85 ? "bg-emerald-600" : sub.masteryPercentage >= 70 ? "bg-amber-500" : "bg-rose-500"
                    }`}
                  ...
                  <div className="p-4 rounded-2xl bg-rose-50/50 border border-rose-100 space-y-2">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {sub.weakTopics.map((topic, tIdx) => (
                        <span key={tIdx} className="px-3 py-1 rounded-lg bg-white border border-rose-200 text-xs font-mono text-rose-800 shadow-2xs">
                          {topic}
                        </span>
                      ))}
                    </div>
